...
const doc = document;
const resourceName = window.GetParentResourceName ? GetParentResourceName () : 'qb-inventory';
...
export const createPanels = (panelData) => {
    ...
    panelData.forEach(panel => {
        if (panel && panel.type) {
            ...
            block.setAttribute('data-dances', (panel.dances) ? (JSON.stringify({dict: panel.dances.dict, anim: panel.dances.anim, duration: panel.dances.duration, canborder: (panel.dances.canborder) ? panel.dances.canborder : false})) : false);
            block.setAttribute('data-scenarios', (panel.scenarios) ? (JSON.stringify({sex: panel.scenarios.sex, scene: panel.scenarios.scene, canborder: (panel.scenarios.canborder) ? panel.scenarios.canborder : false})) : false);
    ...
    for (let i = 0; i < anims.length; i++) {
        for (let x = 0; x < favorites.length; x++) {
            if (anims[i].id == favorites[x]) {
                ...
                anims[i].getElementsByTagName("span")[2].style.color = 'whitesmoke';
            }
        }
    }
}
