...
function handleGive($slot) {
    const data = $slot.data('item');
    const inventory = $slot.parent().attr('data-inventory');
    if (!data) return;
    let amount = parseInt($('#item-amount').val(), 10);
    if (!amount) amount = data.amount;
    $.post(
        'https://qb-inventory/GiveItem',
        JSON.stringify({ inventory, item: data, amount })
    );
}

$(document).on('mousedown', '.item-slot', function (e) {
    if (e.which === 2) handleGive($(this));
});
...
$(document).on("mouseleave", ".item-slot", function (e) {
    e.preventDefault();
    $(".ply-iteminfo-container").hide();
});

$(".clothing").click((targetDataClothing) => {
    $.post("https://qb-inventory/updateClothingDta", JSON.stringify({ clothIdData: targetDataClothing.target.id }));
});
        ...
        if (ItemData.name !== undefined) {
            if (ItemData.name.split("_")[0] == "weapon") {
                if (!$("#weapon-attachments").length) {
                    $("#rob-money").hide()
                    // $(".inv-options-list").append('<div class="inv-option-item" style="font-size:1.2vh" id="weapon-attachments"><p>COMPONENTES</p></div>');
...
$(document).on("click", ".weapon-attachments-back", function (e) {
    ...
    $("#qbcore-inventory").animate({ opacity: 100 }, 0);
    $(".weapon-attachments-container").animate({ opacity: 0 }, 100, function () {
        ...
        function (data) {
            ...
            if (data.AttachmentData !== null && data.AttachmentData !== undefined) {
                if (data.AttachmentData.length > 0) {
                    ...
                    $(".weapon-attachments-title").html('<span style="font-weight: bold; letter-spacing: .1vh;">Este arma no tiene accesorios</span>');
                }
            } else {
                $(".weapon-attachments-title").html('<span style="font-weight: bold; letter-spacing: .1vh;">Este arma no tiene accesorios</span>');
            }
...
function generateDescription(itemData) {
    if (itemData.type === "weapon") {
        ...
        return `<p><strong>Número de Serie: </strong><span>${itemData.info.serie}</span></p>
                <p><strong>Cargador: </strong><span>${ammo}</span></p>
                <p>${itemData.description}</p>`;
    }
    switch (itemData.name) {
        default:
            return `<p>${itemData.description}</p>
        <p><strong> Caducidad: </strong><span>${Math.floor(itemData.info.quality)}</span></p>`;
    }
}
...
function optionSwitch($fromSlot, $toSlot, $fromInv, $toInv, $toAmount, toData, fromData) {
    ...
    if ($toSlot < 6) {
        $toInv.find("[data-slot=" + $toSlot + "]").html('<div class="item-slot-key"><p>' + $toSlot + '</p></div><div class="item-slot-img"><img src="images/' + fromData.image + '" alt="' + fromData.name + '" /></div><div class="item-slot-amount"><p>' + fromData.amount + '</p></div><div class="item-slot-label"><p>' + fromData.label + "</p></div>");
    } else {
        $toInv.find("[data-slot=" + $toSlot + "]").html('<div class="item-slot-img"><img src="images/' + fromData.image + '" alt="' + fromData.name + '" /></div><div class="item-slot-amount"><p>' + fromData.amount + '</p></div><div class="item-slot-label"><p>' + fromData.label + "</p></div>");
    }
    ...
    if ($fromSlot < 6) {
        $fromInv.find("[data-slot=" + $fromSlot + "]").html('<div class="item-slot-key"><p>' + $fromSlot + '</p></div><div class="item-slot-img"><img src="images/' + toData.image + '" alt="' + toData.name + '" /></div><div class="item-slot-amount"><p>' + toData.amount + '</p></div><div class="item-slot-label"><p>' + toData.label + "</p></div>");
    } else {
        $fromInv.find("[data-slot=" + $fromSlot + "]").html('<div class="item-slot-img"><img src="images/' + toData.image + '" alt="' + toData.name + '" /></div><div class="item-slot-amount"><p>' + toData.amount + '</p></div><div class="item-slot-label"><p>' + toData.label + "</p></div>");
    }
...
function swap($fromSlot, $toSlot, $fromInv, $toInv, $toAmount) {
    ...
    if ($toInv.attr("data-inventory") == "player" && $toSlot < 6) {
        $toInv = $(".player-inventory-keys");
        toData = $toInv.find("[data-slot=" + $toSlot + "]").data("item");
    }

    if ($fromInv.attr("data-inventory") == "player" && $fromSlot < 6) {
        $fromInv = $(".player-inventory-keys");
        fromData = $fromInv.find("[data-slot=" + $fromSlot + "]").data("item");
    }
            ...
            if (fromData.amount == $toAmount) {
                ...
                if (newData.name.split("_")[0] == "weapon") {
                    if (!Inventory.IsWeaponBlocked(newData.name)) {
                        ItemLabel = '<div class="item-slot-quality"><div class="item-slot-quality-bar"></div></div><div class="item-slot-label"><p>' + newData.label + "</p></div>";
                    }
                }

                if ($toSlot < 6 && $toInv.attr("data-inventory") == "player") {
                    $toInv.find("[data-slot=" + $toSlot + "]").html('<div class="item-slot-key"><p>' + $toSlot + '</p></div><div class="item-slot-img"><img src="images/' + newData.image + '" alt="' + newData.name + '" /></div><div class="item-slot-amount"><p>' + newData.amount + "</p></div>" + ItemLabel);
                } else if ($toSlot == 41 && $toInv.attr("data-inventory") == "player") {
                    $toInv.find("[data-slot=" + $toSlot + "]").html('<div class="item-slot-key"><p>6</p></div><div class="item-slot-img"><img src="images/' + newData.image + '" alt="' + newData.name + '" /></div><div class="item-slot-amount"><p>' + newData.amount + "</p></div>" + ItemLabel);
                } else {
                    $toInv.find("[data-slot=" + $toSlot + "]").html('<div class="item-slot-img"><img src="images/' + newData.image + '" alt="' + newData.name + '" /></div><div class="item-slot-amount"><p>' + newData.amount + "</p></div>" + ItemLabel);
                }
                ...
                if ($fromInv.attr("data-inventory").split("-")[0] == "itemshop") {
                    ...
                    if (newDataFrom.name.split("_")[0] == "weapon") {
                        if (!Inventory.IsWeaponBlocked(newDataFrom.name)) {
                            ItemLabel = '<div class="item-slot-quality"><div class="item-slot-quality-bar"></div></div><div class="item-slot-label"><p>' + newDataFrom.label + "</p></div>";
                        }
                    }

                    if ($fromSlot < 6 && $fromInv.attr("data-inventory") == "player") {
                        $fromInv.find("[data-slot=" + $fromSlot + "]").html('<div class="item-slot-key"><p>' + $fromSlot + '</p></div><div class="item-slot-img"><img src="images/' + newDataFrom.image + '" alt="' + newDataFrom.name + '" /></div><div class="item-slot-amount"><p>' + newDataFrom.amount + "</p></div>" + ItemLabel);
                    } else if ($fromSlot == 41 && $fromInv.attr("data-inventory") == "player") {
                        $fromInv.find("[data-slot=" + $fromSlot + "]").html('<div class="item-slot-key"><p>6</p></div><div class="item-slot-img"><img src="images/' + newDataFrom.image + '" alt="' + newDataFrom.name + '" /></div><div class="item-slot-amount"><p>' + newDataFrom.amount + "</p></div>" + ItemLabel);
                    } else {
                        $fromInv.find("[data-slot=" + $fromSlot + "]").html('<div class="item-slot-img"><img src="images/' + newDataFrom.image + '" alt="' + newDataFrom.name + '" /></div><div class="item-slot-amount"><p>' + newDataFrom.amount + "</p></div>" + ItemLabel);
                    }
            ...
            if (fromData.amount == $toAmount) {
                ...
                if (fromData.name.split("_")[0] == "weapon") {
                    if (!Inventory.IsWeaponBlocked(fromData.name)) {
                        ItemLabel = '<div class="item-slot-quality"><div class="item-slot-quality-bar"></div></div><div class="item-slot-label"><p>' + fromData.label + "</p></div>";
                    }
                }

                if ($toSlot < 6 && $toInv.attr("data-inventory") == "player") {
                    $toInv.find("[data-slot=" + $toSlot + "]").html('<div class="item-slot-key"><p>' + $toSlot + '</p></div><div class="item-slot-img"><img src="images/' + fromData.image + '" alt="' + fromData.name + '" /></div><div class="item-slot-amount"><p>' + fromData.amount + "</p></div>" + ItemLabel);
                } else if ($toSlot == 41 && $toInv.attr("data-inventory") == "player") {
                    $toInv.find("[data-slot=" + $toSlot + "]").html('<div class="item-slot-key"><p>6</p></div><div class="item-slot-img"><img src="images/' + fromData.image + '" alt="' + fromData.name + '" /></div><div class="item-slot-amount"><p>' + fromData.amount + "</p></div>" + ItemLabel);
                } else {
                    $toInv.find("[data-slot=" + $toSlot + "]").html('<div class="item-slot-img"><img src="images/' + fromData.image + '" alt="' + fromData.name + '" /></div><div class="item-slot-amount"><p>' + fromData.amount + "</p></div>" + ItemLabel);
                }
                ...
                if (toData != undefined) {
                    ...
                    if (toData.name.split("_")[0] == "weapon") {
                        if (!Inventory.IsWeaponBlocked(toData.name)) {
                            ItemLabel = '<div class="item-slot-quality"><div class="item-slot-quality-bar"></div></div><div class="item-slot-label"><p>' + toData.label + "</p></div>";
                        }
                    }

                    if ($fromSlot < 6 && $fromInv.attr("data-inventory") == "player") {
                        $fromInv.find("[data-slot=" + $fromSlot + "]").html('<div class="item-slot-key"><p>' + $fromSlot + '</p></div><div class="item-slot-img"><img src="images/' + toData.image + '" alt="' + toData.name + '" /></div><div class="item-slot-amount"><p>' + toData.amount + "</p></div>" + ItemLabel);
                    } else if ($fromSlot == 41 && $fromInv.attr("data-inventory") == "player") {
                        $fromInv.find("[data-slot=" + $fromSlot + "]").html('<div class="item-slot-key"><p>6</p></div><div class="item-slot-img"><img src="images/' + toData.image + '" alt="' + toData.name + '" /></div><div class="item-slot-amount"><p>' + toData.amount + "</p></div>" + ItemLabel);
                    } else {
                        $fromInv.find("[data-slot=" + $fromSlot + "]").html('<div class="item-slot-img"><img src="images/' + toData.image + '" alt="' + toData.name + '" /></div><div class="item-slot-amount"><p>' + toData.amount + "</p></div>" + ItemLabel);
                    }
                ...
                if ($toSlot < 6 && $toInv.attr("data-inventory") == "player") {
                    $toInv.find("[data-slot=" + $toSlot + "]").html('<div class="item-slot-key"><p>' + $toSlot + '</p></div><div class="item-slot-img"><img src="images/' + newDataTo.image + '" alt="' + newDataTo.name + '" /></div><div class="item-slot-amount"><p>' + newDataTo.amount + "</p></div>" + ItemLabel);
                } else if ($toSlot == 41 && $toInv.attr("data-inventory") == "player") {
                    $toInv.find("[data-slot=" + $toSlot + "]").html('<div class="item-slot-key"><p>6</p></div><div class="item-slot-img"><img src="images/' + newDataTo.image + '" alt="' + newDataTo.name + '" /></div><div class="item-slot-amount"><p>' + newDataTo.amount + "</p></div>" + ItemLabel);
                } else {
                    $toInv.find("[data-slot=" + $toSlot + "]").html('<div class="item-slot-img"><img src="images/' + newDataTo.image + '" alt="' + newDataTo.name + '" /></div><div class="item-slot-amount"><p>' + newDataTo.amount + "</p></div>" + ItemLabel);
                }
...
(() => {
    ...
    Inventory.Open = function (data) {
        ...
        $(".player-inventory-keys").find(".item-slot").remove();
    ...
    Inventory.UseItem = function (data) {
        ...
        $("#itembox-action").html("<p class='bebas'>USADO</p>");
    ...
    Inventory.itemBox = function (data) {
        ...
        //             <div class="itembox-title">${data.item.label}</div>
        //             <div class="itembox-desc">x${data.amount} ${type}</div>
        //         </div>
        //     </div>
        // `) Juliroo
        ...
        var $itembox = $(`
            <div class="itembox-action">
                ...
                <div class="itembox-col">
                    <div class="itembox-title">${data.item.label}</div>
                    <div class="itembox-desc">${type}</div>
                </div>
            </div>
        `)
        ...
        window.addEventListener("message", function (event) {
            switch (event.data.action) {
                ...
                case "toggleHotbar":
                    ...
                    break;
                case "RobMoney":
                    // $(".inv-options-list").append('<div class="inv-option-item" style="font-size:1vh" id="rob-money"><p>ROBAR</br>DINERO</p></div>');
...
$(document).keyup((e) => {
    if (e.target.matches("#item-amount") == false) {
        $.post('https://qb-inventory/enablecontrols');
    }
});

$("#item-amount").on("click", function () {
    $.post('https://qb-inventory/disablecontrols');
});

$(document).on("mouseenter", ".item-slot", function (e) {
    e.preventDefault();
    if ($(this).data("item") != null) {
        let bottom = ($(window).height() - ($(this).offset().top + $(this).outerHeight()));
        let right = ($(window).width() - ($(this).offset().left + $(this).outerWidth()));
        $(".ply-iteminfo-container").css({ "bottom": bottom, "right": right, "transform": "translate(-3.5vw, -3.5vw)" });
        $(".ply-iteminfo-container").show();
        FormatItemInfo($(this).data("item"));
    } else {
        $(".ply-iteminfo-container").hide();
    }
