...
function log(l) -- Just a simple logging thing, to easily log all kinds of stuff.
	...
	if not ConfigClothing.Debug then return end
...
function Text(x, y, scale, text, colour, align, force, w)
	...
	local colour = colour or ConfigClothing.GUI.TextColor
	SetTextFont(ConfigClothing.GUI.TextFont)
	SetTextJustification(align)
	SetTextScale(scale, scale)
	SetTextColour(colour[1], colour[2], colour[3], 255)
	if ConfigClothing.GUI.TextOutline then SetTextOutline() end	
...
function Lang(what)
	local Dict = Locale[ConfigClothing.Language]
...
AddEventHandler('dpc:ToggleMenu', function()
	...
	if MenuOpened then SoundPlay("Open") SetCursorLocation(ConfigClothing.GUI.Position.x, ConfigClothing.GUI.Position.y) else SoundPlay("Close") end
end)
...
RegisterNUICallback("toggleCloth", function(command)
