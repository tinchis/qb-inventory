...
RegisterNUICallback("getActualState", function(data, cb) 
	local sended = {}
	for k,v in pairs(LastEquipped) do
...
for k,v in pairs(ConfigClothing.Commands) do
...
if ConfigClothing.ExtrasEnabled then
	for k,v in pairs(ConfigClothing.ExtraCommands) do
		RegisterCommand(k, function() 
			v.Func()  
			local sended = {}
			for k,v in pairs(LastEquipped) do
